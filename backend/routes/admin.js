...
router.put("/editEmploy/:ID", async (req, res) => {
  try {
    const { Name, Phone, Email, Address, ChargePH, Designation, Active } =
      req.body;
    ...
    const updatedEmploy = await EmployDB.findByIdAndUpdate(
      req.params.ID,
      { $set: newEmploy },
      { new: true }
    );
...
router.put("/editSchedule/:ID", async (req, res) => {
  try {
    ...
    const updatedSchedule = await ScheduleDB.findByIdAndUpdate(
      req.params.ID,
      { $set: newSchedule },
      { new: true }
    );
...
router.get("/idPresentSchedule/:ID", async (req, res) => {
  ...
  try {
    const ScheduleList = await ScheduleDB.find({
      EmpID: id,
      CurrDate: { $gte: threshold },
    });
    res.json(ScheduleList);
...
router.get("/idAllSchedule/:ID", async (req, res) => {
  ...
  try {
    const ScheduleList = await ScheduleDB.find({ EmpID: id }).sort({
      Entry: 1,
    });
    res.json(ScheduleList);
    // console.log(ScheduleList);
...
router.get("/getDashboardData", async (req, res) => {
  try {
    ...
    const scheduleTodayList = await ScheduleDB.find({
      CurrDate: { $gte: threshold },
    });
    var activeUserCount = 0,
      loggedUserCount = 0,
      scheduleCount = scheduleTodayList.length;
    ...
    res.send({
      ...
      loggedUser: loggedUserCount,
      ScheduleCountToday: scheduleCount,
    });
  } catch (error) {
    console.log(error);
  }
...
router.post("/verifyUserToken", async (req, res) => {
  try {
    ...
    const bo = user[0].currToken === toverify;

    if (user[0].currToken === toverify) {
      statusveried = 200;
    }
    return res
      .status(statusveried)
      .json({ success: "Verification done", dataUser: user, verified: bo });
  } catch (error) {
    console.log(error);
...
router.put("/changePassword", async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json({ success, error: "Please try to use correct credentials" });
    }
    // console.log(user);
